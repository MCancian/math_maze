...
reproducible), `set_piece` (optional hand-placed overlay), `monster_spawn` override,
`star_thresholds` + `stars_for(elapsed)`. Progression list is `GameManager.LEVELS`
(gen_01…gen_05).
...
`entrance`, `exit` (BFS-farthest), `deadends`, `links` (cell adjacency), and `dist`
(distance-from-entrance map).

## Building — `LevelBuilder` (`scenes/levels/level_builder.gd`)

`LevelBuilder.build(root, info, maze_cfg, set_piece, level_data=null) -> int` (keys
placed). Spawns CSGBox floor + walls (`CELL=4`, `WALL_H=3`), door at the exit, keys at
the farthest dead-ends (falls back to farthest cells when braiding leaves too few),
optional `set_piece` overlay, optional monster when gated on, and player at the entrance
(added last so the HUD reads the final count).
